...
export default function BookTablePage() {
  ...
  const fetchAvailableSlots = async (selectedDate) => {
    setIsLoading(true);
    try {
      const response = await fetch(`https://table-booking-hzcopvvm2-vasundharaba24s-projects.vercel.app/availability?date=${selectedDate}`);
      if (!response.ok) {
  ...
  const requestVerificationCode = async () => {
    ...
    try {
      const response = await fetch('https://table-booking-hzcopvvm2-vasundharaba24s-projects.vercel.app/send-verification-code', {
        method: 'POST',
  ...
  const verifyCode = async () => {
    ...
    try {
      const response = await fetch('https://table-booking-hzcopvvm2-vasundharaba24s-projects.vercel.app/verify-code', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://table-booking-hzcopvvm2-vasundharaba24s-projects.vercel.app/book-table', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const sendConfirmationEmail = async (email, date, time) => {
    try {
      const response = await fetch('https://table-booking-hzcopvvm2-vasundharaba24s-projects.vercel.app/send-confirmation-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
